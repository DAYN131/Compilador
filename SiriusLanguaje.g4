...
variableDeclaration: (VAR | VAL) IDENTIFIER (COLON type)? ASSIGN expression SEMICOLON;

functionDeclaration: 
    FUN IDENTIFIER LPAREN parameters? RPAREN (COLON type)? block
    ;
...
ifStatement: IF LPAREN expression RPAREN block (ELSE IF LPAREN expression RPAREN block)* (ELSE block)?;

forStatement: 
    FOR LPAREN (variableDeclaration | expression)? SEMICOLON 
    expression? SEMICOLON 
...
// Lexer rules (igual que en la respuesta anterior)
TRUE: 'true';
FALSE: 'false';
AND: 'and';
OR: 'or';
